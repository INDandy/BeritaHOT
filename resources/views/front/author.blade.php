			...
			<a href="{{route('front.details', $news->slug)}}" class="card">
				<div
					...
					<div class="flex flex-col gap-[6px]">
						<h3 class="text-lg leading-[27px] font-bold"> {{substr($news->name,0,50)}}{{strlen(string: $news->name)>50?'...':''}}</h3>
						<p class="text-sm leading-[21px] text-[#A3A6AE]">{{$news->created_at->format('M d, Y')}}</p>
					</div>
				</div>
			</a>
            @empty
            <p class="text-center text-lg font-semibold">No news available from this author</p>
            @endforelse
		</div>
	</section>
